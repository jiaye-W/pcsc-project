...
ExpectedValue::ExpectedValue(RNG rng, VectorFunction vectorFunction)
{
    mRNG = rng;
    mVectorFunction = vectorFunction;
}

std::vector<double> ExpectedValue::Compute(int numberOfSamples)
{
    // Initialise
    ...
    std::vector<double> samples = mRNG.GenerateSamples(numberOfSamples);
...
RNG ExpectedValue::GetRNG() const { return mRNG; }

VectorFunction ExpectedValue::GetVectorFunction() const { return mVectorFunction; }

void ExpectedValue::SetRNG(const RNG rng) { mRNG = rng; }
